...
def JDR(xVal1,xVal2, xn, xp, W, lifetimeN, mobilityN, lifetimeP, mobilityP,nSide, pSide, iSide, percent):
    ...
    densitiesElectrons = [4.67e-8]
    densitiesHoles=[4.67e-8]
    loc = 0

    def scaled_index_lookup(value, data_list):
        if not (1 <= value <= 99):
            raise ValueError("Value must be between 1 and 99.")
        if len(data_list) == 0:
            raise ValueError("List must contain at least one element.")
    
        # Scale value (1-99) to index (0 to len(data_list)-1)
        scale = (value - 1) / 98  # Normalize to 0-1
        index = int(round(scale * (len(data_list) - 1)))
        return data_list[index]

    def findLifetimeElectrons(place):
        concentrations = scaled_index_lookup(place, densitiesElectrons)
    ...
    def Ip1(x):
        ...
        return (eF * mp.exp(abs(float(accumulation))))
    ...
    def QIp(x, num):
        Q = Qp(x)
        accumulation = ordered_integrate(intElecP, -xn, x)
        print('accumulation 2 '+ str(accumulation))

        accumulation = mp.mpf(accumulation)
        I = mp.exp(abs(float(accumulation)))
    ...
    def finalDensity(start, end):
        allPosN = np.linspace(start, end, 50)
        allPosP = np.linspace(end, start, 50)
        for i in allPosN:
            densitiesElectrons.append((n(i)))
        for i in allPosP:
            densitiesHoles.append((p(i)))

    finalDensity(float(xVal1,) float(xVal2))
    ...
    return ({"Electron density":densityElectrons, "Hole density": abs(densityHoles),"Emin":eMin,"Efield":abs(avgField),"xn":xn,'xp':xp,'W':W, "nSide":nSide, "pSide":pSide, "ratio":(eMin/abs(avgField)), "percent":percent, "Current":totalCurrent, "Current-Electron": currentElectron, "Current-Hole": currentHole})
