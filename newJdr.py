...
def bulkRecombinationLifetimeMaj(concMinority,concMajority, type):
    ...
    return majorityTotalBulkLifetime
...
def totalMajorityLength(temp, typeOfMajority, majorityConc):
    ...
    totalLength = mp.sqrt(diffusion*totalMajorityLifetimeVal)
...
def JDR(xVal1,xVal2, xn, xp, W, lifetimeN, mobilityN, lifetimeP, mobilityP,nSide, pSide, iSide, percent):
    ...
    def calculateLifetimeN(x):
        distance = lastXN[0]-(x+xn)
        if distance == 0:
            return None
        # print("Electron")
        # print(x, distance)
        lastXN.pop()
        lastXN.append(x+xn)
        concElectron = abs(lastConcN[0])
        concHole=abs(lastConcP[0])
        concentrationElectron = concElectron/distance
        concentrationHole = concHole/distance
        # print(concHole, concElectron)
        lifetime = bulkRecombinationLifetimeMaj(concentrationElectron, concentrationHole, 'n')
        everyLifetimeN.append(lifetime)
        return lifetime
    
    def calculateLifetimeP(x):
        distance = lastXP[0]-(x+xn)
        # print("Hole")
        # print(x, distance)
        if distance == 0:
            return None
        ...
        lifetime = bulkRecombinationLifetimeMaj(concentrationHole, concentrationElectron, 'p')
        everyLifetimeP.append(lifetime)
        return lifetime
    ...
    def intElecN(x):
        ...
        epsilon = avgField/1000  # Adjust based on scale (e.g., 1000 V/m)
        # print(epsilon)
        if abs(eF) < abs(epsilon):
            eF = epsilon

        lifetimeN = calculateLifetimeN(x)
        # print("Lifetime:"+str(lifetimeN))
    ...
    def intElecP(x):
        ...
        lifetimeP = calculateLifetimeP(x)
        if lifetimeP is not None:
            lifetimeP=float(lifetimeP)

        # print(lifetimeP)

        if lifetimeP is not None and lifetimeP < 1e-15:
            print(f"⚠️ Short τP at x={x:.2e}: τP = {lifetimeP:.2e}")
    ...
    def In1(x):
        ...
        print("Accumulation n:"+str(accumulation))
    ...
    def Ip1(x):
        ...
        accumulation = ordered_integrate(intElecP, -xn, x)
        accumulation = mp.mpf(accumulation)

        print("Accumulation P:"+str(accumulation))
        ...
        print("mp.exp(-accumulation: "+ str(mp.exp(abs(accumulation))))
        print("eF:"+str(eF))
    ...
    def n(x):
        ...
        lastXN = [depWidth]
        lastConcN = [4.67e-8]
        val2= abs(ordered_integrate(QIn, xp+W, x))
    ...
    lifetimeN = 0
    for i in everyLifetimeN:

        lifetimeN+=i
    lifetimeN/=len(everyLifetimeN)
    
    lifetimeP = 0
    for i in everyLifetimeP:

        lifetimeN+=i
    lifetimeN/=len(everyLifetimeP)
...
W/=10000
xp/=10000
print("Electron density: " + str(JDR(-xn,xp+W, xn, xp, W, 10e-9, 1000, 1e-9, 40,1e17, 1e17, 1e15, 0.5)))
